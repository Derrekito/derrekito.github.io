...
```mermaid
graph TD
    Stack["<b>Stack</b><br/>temp (local variable)"]
    Heap["<b>Heap</b><br/>Node struct<br/>data: 10<br/>next: NULL<br/>Address: 0x7f8a"]
...
```mermaid
graph TD
    mainStack["<b>main()'s Stack</b>"]
    head1["head"]
    tail1["tail"]
    node1["Node 1<br/>data: 10"]
    node2["Node 2<br/>data: 100"]

    mainStack --> head1
    mainStack --> tail1
    head1 -->|points to| node1
    node1 -->|next| node2
    node2 -->|next| NULL1["..."]
```

**prepend() receives pointer-to-pointer:**

The function receives the address of `head` in main's stack, not just the node pointer. This enables modification of the original `head` variable.

**After (*head_ptr) = new_node:**

```mermaid
graph TD
    mainStack2["<b>main()'s Stack</b>"]
    head2["head"]
    tail2["tail"]
    newNode["New Node<br/>data: 200"]
    oldNode1["Old Node 1<br/>data: 10"]
    oldNode2["Old Node 2<br/>data: 100"]

    mainStack2 --> head2
    mainStack2 --> tail2
    head2 -->|now points to| newNode
    newNode -->|next| oldNode1
    oldNode1 -->|next| oldNode2
    oldNode2 -->|next| NULL2["..."]
```
...
graph TD
    head["head pointer"]
    tail["tail pointer"]
    node["Node<br/>data: 10<br/>next: NULL"]
...
**Step 1-2: Before append operation:**

```mermaid
graph TD
    tail["tail pointer"]
    node1["Node<br/>data: 10<br/>next: NULL"]

    tail -->|points to| node1
```

**Step 3: After creating new node and linking:**

```mermaid
graph TD
    tail["tail pointer"]
    node1["Node<br/>data: 10<br/>next: ?"]
    node2["Node<br/>data: 100<br/>next: NULL"]

    tail -->|still points to| node1
    node1 -->|next now links| node2
```

**Step 4: After updating tail pointer:**

```mermaid
graph TD
    tail["tail pointer"]
    node1["Node<br/>data: 10"]
    node2["Node<br/>data: 100<br/>next: NULL"]

    node1 -->|next| node2
    tail -->|now points to| node2
```
