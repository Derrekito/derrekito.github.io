...
```mermaid
graph TD
    A["Coinbase API<br/>(REST Client)"] -->|Raw Data| B["Data Pipeline<br/>(DataFrame)"]
    B -->|OHLCV Data| C["Indicators<br/>(Ichimoku)"]
    C -->|Indicator Values| D["Signal Generator"]
    D -->|Trading Signals| E["Risk Management"]
    E -->|Risk-Filtered Signals| F["Trade Execution"]
```
