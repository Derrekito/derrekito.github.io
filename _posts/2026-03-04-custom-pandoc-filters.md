---
title: "Executable Notebooks (Part 2): Custom Pandoc Filters for Technical Documentation"
date: 2026-03-04
...
> **Note:** Code examples in this post are simplified for illustration. The actual filters include additional error handling and edge cases. A complete starter template is [available on Gumroad](https://derrekito.gumroad.com/l/jtgyzf).
...
In Lua filters:
- `doc.meta.title` → `"Analysis Report"` (MetaInlines)
- `doc.meta.notebook` → `true` (MetaBool)
- `doc.meta.toc` → `true` (MetaBool)
- `doc.meta.custom_field` → `"any value"` (MetaInlines)
...
The full filter implementations are included in the [starter template on Gumroad](https://derrekito.gumroad.com/l/jtgyzf).
