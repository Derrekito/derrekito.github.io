...
A [previous post on dmenu keybinding cheatsheets](/posts/dmenu-searchable-keybinding-cheatsheets/) established the core architecture: heredoc-embedded data, category extraction via grep/sed, and two-level dmenu navigation. That implementation focused exclusively on keyboard shortcuts for tools like Neovim, tmux, and i3.
...
```mermaid
graph TD
    A["1. HEREDOC DATABASE<br/>Category Name<br/>key/command/item → description<br/>..."] --> B["2. CATEGORY EXTRACTION<br/>grep '^# ' | sed 's/# //'"]
    B --> C["3. TWO-LEVEL NAVIGATION<br/>Category selection → Entry display"]
```
