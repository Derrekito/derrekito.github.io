...
```mermaid
graph LR
    A["input.md"] --> B["include-files.lua<br/>(expand includes)"]
    B --> C["readme-only.lua<br/>(strip README content)"]
    C --> D["notebook-toggle.lua<br/>(control code visibility)"]
    D --> E["nobreak-codeblock.lua<br/>(prevent page breaks)"]
    E --> F["md-links-to-refs.lua<br/>(markdown to LaTeX refs)"]
    F --> G["pandoc-mermaid.py<br/>(render diagrams)"]
    G --> H["pandoc-minted.py<br/>(syntax highlighting)"]
    H --> I["output"]
```
