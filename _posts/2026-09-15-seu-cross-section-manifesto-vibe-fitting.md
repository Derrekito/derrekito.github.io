---
title: "SEU Cross-Section Analysis (Part 0): A Methodological Manifesto Against Vibe Fitting"
date: 2026-09-15
...
```mermaid
graph TD
    A["Data Characterization"] --> B["Assess Data Properties"]

    B --> B1["✓ Total events<br/>✓ Zero observations?<br/>✓ Minimum count<br/>✓ Data points<br/>✓ Degrees of freedom"]

    B1 --> C["Method Selection"]

    C --> D1["Decision 1:<br/>MLE Variant"]
    D1 --> D2{has_zeros?}
    D2 -->|Yes| D3["MLE-WithZeros"]
    D2 -->|No| D4{N ≥ 50?}
    D4 -->|Yes| D5["MLE-Standard<br/>Hessian covariance"]
    D4 -->|No| D6["MLE-SmallSample<br/>defer to bootstrap"]

    C --> E1["Decision 2:<br/>Bootstrap Variant"]
    E1 --> E2{N ≥ 50 AND<br/>min_count ≥ 5?}
    E2 -->|Yes| E3["Full Bootstrap<br/>10,000 iterations"]
    E2 -->|No| E4["Conservative<br/>20,000 iterations"]

    C --> F1["Decision 3:<br/>Confidence Interval"]
    F1 --> F2{N ≥ 50 AND<br/>no zeros?}
    F2 -->|Yes| F3["BCA<br/>second-order accurate"]
    F2 -->|No| F4["Percentile<br/>robust"]

    C --> G1["Decision 4:<br/>Zero Event Handler"]
    G1 --> G2{has_zeros?}
    G2 -->|Yes| G3["Apply 3.7/Φ<br/>upper limit rule"]
    G2 -->|No| G4["No handler needed"]

    C --> H1["Decision 5:<br/>Goodness-of-Fit Test"]
    H1 --> H2{DoF ≥ 3?}
    H2 -->|Yes| H3["Run deviance test"]
    H2 -->|No| H4["Skip<br/>insufficient DoF"]
```
...
*This post is Part 0 of the SEU Cross-Section Analysis series. Next: [MLE for Weibull Cross-Sections](/posts/seu-cross-section-mle-weibull)*
